...
def compile_device(
    name: str, firmware_version: str | None = None, package_owner: str | None = None
) -> None:
...
def main() -> None:
    ...
    parser.add_argument(
        "--release-only",
        action="store_true",
        help="Compile only release devices when compiling all devices.",
    )
    parser.add_argument(
        "--firmware-version", help="Value to pass as the ESPHome firmware_version substitution."
    )
    parser.add_argument(
        "--package-owner", help="Value to pass as the ESPHome package_owner substitution."
    )
    args = parser.parse_args()

    names = (
        device_names(release_only=args.release_only)
        if args.all or not args.devices
        else args.devices
    )
    for name in names:
        compile_device(
            name, firmware_version=args.firmware_version, package_owner=args.package_owner
        )
